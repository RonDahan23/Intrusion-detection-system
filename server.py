...
def sniff_packet():
    sniff(ifaces="eth0", prn = lambda x: x.show())
...
def get_packet_info(packet):
    try:
        ...
        if IP in packet:
            ...
            urgent_flag = 0  # Initialize urgent_flag outside the if statement

            if protocol_name == 'TCP' and TCP in packet:  
                urgent_flag = 1 if packet[TCP].flags & 0x20 else 0  

            num_failed_logins, Logged_In, su_attempted = count_failed_logins(packet)

            root_shell = 1 if packet[protocol_name].dport == 22 else 0
                
            num_root = count_root_accesses(packet, protocol_name) 

            num_file_creations = packet_indicates_file_creation(packet)

            num_shells = count_shell_accesses(packet, protocol_name) 

            num_access_files = count_file_accesses(packet, protocol_name)

            guest_login = is_guest_login(packet)

            host_login = is_host_login(packet)

            count, srv_count = count_connections(packet)

            Serror_rate, Srv_serror_rate = Serror_rate_calculate(packet, count, srv_count)

            Rerror_rate, Srv_Rerror_rate = Check_REJ(packet, count, srv_count)

            Same_srv_rate = srv_count / count if count > 0 else 0

            Diff_srv_rate = calculate_diff_srv_rate(packet, count)
